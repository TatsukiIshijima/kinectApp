...
# endif /* USE_THRAD */
 ...
 public:
  ...
  // 人領域のみを点郡として返す関数
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr convertBodyIndexToPointXYZ()
  {
	  updateDepthFrame();
	  updateBodyIndexFrame();
	  ...
	  cloud->width = static_cast<uint32_t>(bodyIndexWidth);
	  cloud->height = static_cast<uint32_t>(bodyIndexHeight);
	  ...
	  {
		  for (int x = 0; x < bodyIndexWidth; x++, pt++)
		  {
			  pcl::PointXYZRGB point;

			  int depthIndex = (y * bodyIndexWidth + x);
			  
			  // 人が検出した画素(Buffer)を取り出す
  ...
  void updateBodyFrame() {
	...
	// 床の法線ベクトル算出
	bodyFrame->get_FloorClipPlane(&floor);
  }
